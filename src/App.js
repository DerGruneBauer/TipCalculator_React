...
function App() {
  ...
  const collectNumberPeople = (e) => setNumberPeople(Number(e.target.value));
  
  const collectTipPercentage = (e) => {
      setTipPercentage(parseFloat(e.currentTarget.getAttribute('value')));
  }

  const collectBillAmount = (e) => setBill(Number(e.target.value));

  const collectCustomTip = (e) => {
    setTipPercentage(e.target.value/100);
  }

  const calculateTotalWithTip = () => {
    return bill + calculateTotalTip();
  }
  ...
  const calculateTipPerPerson = () => {
    return Math.round(calculateTotalTip() / numberPeople);
  }

  const calculateTotalPerPerson = () => {
    return Math.round(calculateTotalWithTip() / numberPeople);
  }
