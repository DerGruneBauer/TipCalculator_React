...
function App() {
  ...
  const collectNumberPeople = (e) => {
    setNumberPeople(Number(e.target.value));
  }

  const collectTipPercentage = (e) => {
    setTipPercentage(parseFloat(e.target.getAttribute('value')));
  }

  const collectBillAmount = (e) => {
  ...
  return (
    <div className="App">
      ...
      <CalculatorContainer onResetClick={resetForm} totalPer={calculateTotalPerPerson()} tipTotalPer={calculateTipPerPerson()} onPeopleChange={collectNumberPeople} onBillChange={collectBillAmount} onTipClick={collectTipPercentage} bill={bill} tip={tipPercentage} people={numberPeople} className="calculatorContainer"/>
    </div>
  );
}
