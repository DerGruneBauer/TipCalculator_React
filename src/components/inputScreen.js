...
const InputScreen = (props) => {
    ...
    const updateActiveStatus = (id) => { 
        const updatedActiveButtons = tipButtons.map((button) => {
          return button.id === id ? { ...button, isActive: true } : {...button, isActive: false };
         });
         setTipButtons(updatedActiveButtons);
    }

    const[tipButtons, setTipButtons] = useState([

        { id: 1, textValue: "5%", value: .05, isActive: false },
        { id: 2, textValue: "10%", value: .1, isActive: false },
        { id: 3, textValue: "15%", value: .15, isActive: false },
        { id: 4, textValue: "25%", value: .25, isActive: false },
        { id: 5, textValue: "50%", value: .50, isActive: false },

    ])

  const mappedButtons = tipButtons.map((button) => (
    <PercentageButton 
      key={button.id}
      id={button.id}
      onClickTip={updateActiveStatus}
  ...
  return (
    <div className={styles.inputScreen}>
      ...
      <div className={styles.selectTip}>
        {mappedButtons}
        <div className={styles.customInputDiv}>
          <input className={`customInput ${styles.customInput}`} onChange={(e) => props.onCustomClick(e)} placeholder="Custom" />
        </div>
      </div>
